...
from data.dataset import ECGzheyi_two_Base
...
def main(args):
    ...
    if args.group_analysis_csv:
        ...
        for tup in zheyi_data:
            if str(tup[2]).split('/')[-1].replace('.npz', '') in group_eid:
                zheyi_data_grouped.append(tup)
        zheyi_data = zheyi_data_grouped
        print(f'After filtering with group analysis CSV, dataset size: {len(zheyi_data)}')
    # shuffle data
    random.shuffle(zheyi_data)
    # split 6:2:2
    total_num = len(zheyi_data)
    train_data = zheyi_data[:int(0.6*total_num)]
    valid_data = zheyi_data[int(0.6*total_num):int(0.8*total_num)]
    test_data = zheyi_data[int(0.8*total_num):]
    # dataset
    train_set = ECGzheyi_two_Base(data=train_data,isTrain=True,args=args)
    valid_set = ECGzheyi_two_Base(data=valid_data,isTrain=False,args=args)
    test_set = ECGzheyi_two_Base(data=test_data,isTrain=False,args=args)
    all_set = ECGzheyi_two_Base(data=zheyi_data,isTrain=False,args=args)
    ...
    if not args.only_test:
        ...
        msg = model.load_state_dict(ckpt['model'])
        optimal_threshold = ckpt['metrics']['optimal_threshold']
        if args.threshold_method == 'normal':
