...
def main(args):
    ...
    df_all = pd.read_csv('/home/dingzhengyao/Work/ECG_CMR_TAR/ECG_CMR_Rework/ECG_CMR_CMAI/data/HafoData_adjust2/norm_process/HarvardECG_step5.csv')
    if args.group_analysis_csv:
        ...
        group_eid = set(group_csv['FileName'].astype(str).tolist())
        df_all = df_all[df_all['FileName'].astype(str).isin(group_eid)].reset_index(drop=True)
    # dataset
    ...
    print(f"df_all test dataset size: {len(test_set)}")
