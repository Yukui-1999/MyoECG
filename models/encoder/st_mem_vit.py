...
class ST_MEM_ViT(nn.Module):
    ...
    def forward_encoding(self, series):
        ...
        # print(f"Shape after rearranging: {x.shape}") # Shape after rearranging: torch.Size([b, 12, 32, 768])
        x = x[:, :, 1:-1, :]
        self.latent = x
        x = torch.mean(x, dim=(1, 2))
        return self.norm(x)

    def forward(self, series, return_latent=False):
        x = self.forward_encoding(series)
        if return_latent:
            return self.head(x), self.latent
        return self.head(x)
