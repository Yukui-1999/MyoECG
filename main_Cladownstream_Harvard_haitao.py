...
def main(args):
    ...
    if args.input_modality == 'ECG':
        if args.ecg_model == 'stmem':
            ...
            model.to(args.device)
            ckpt = torch.load('/mnt/sda1/dingzhengyao/Work/ECG_CMR_CMAI/ECG_CMR_align_haitao_HenanAlignAfterUKBAlign/ECG_STME_CMR_base_bs256_lr0.001_optadamw_clip0.05_seed42_FreezeCMRTrue_ECGmodelfg_clep_LossCLIPLoss/best-loss.pth',
                            map_location='cpu')['model']
            ckpt_remove_keys = ['model.fc.weight', 'model.fc.bias']
            msg = model.load_state_dict({k: v for k, v in ckpt.items() if k not in ckpt_remove_keys},strict=False)
            print(f'Load pre-trained aligned ECG model: {msg}')
