...
def get_args_parser():
    ...
    parser.add_argument('--group_analysis_csv', default=None, type=str, help='group analysis csv path, should contain a column named "Unnamed: 0" with eids to be included in the test set')
    return parser
...
def main(args):
    ...
    if args.group_analysis_csv:
        ...
        group_eid = set(group_csv['Unnamed: 0'].astype(str).tolist())
        test_excel = pd.read_excel(os.path.join(Henan_data_root,'test.xlsx'))
        test_excel = test_excel[test_excel['Unnamed: 0'].astype(str).isin(group_eid)]
        test_set = HeNan_ECGBaseDis(data_excel=test_excel,isTrain=False,args=args)
