...
from data.dataset import HeNan_ECGBaseDis,ECGzheyi_three_Base,ECGBaseMIMIC_CMthree,Harvard_ECGBaseDis,ShaoyifuCardiomyopathyDataset
...
def get_args_parser():
    ...
    parser.add_argument('--cm_three_dataset', default='Harvard', type=str, help='dataset for cm three classification: Henan, Zheyi, MIMIC, Harvard')
...
def main(args):
    ...
    if args.cm_three_dataset == 'Harvard':
        
        all_df = pd.read_csv('/home/dingzhengyao/Work/ECG_CMR_TAR/ECG_CMR_Rework/ECG_CMR_CMAI/data/HafoData/Norm_process/HarvardECG_step6.csv')
        train_df, temp_df = train_test_split(all_df, test_size=0.3, random_state=seed, shuffle=True)
        val_df, test_df   = train_test_split(temp_df, test_size=0.5, random_state=seed, shuffle=True)
        train_df = train_df.reset_index(drop=True)
        val_df   = val_df.reset_index(drop=True)
        test_df  = test_df.reset_index(drop=True)
        train_set = Harvard_ECGBaseDis(data_df=train_df,isTrain=True,args=args)
        valid_set = Harvard_ECGBaseDis(data_df=val_df,isTrain=False,args=args)
        test_set  = Harvard_ECGBaseDis(data_df=test_df,isTrain=False,args=args)
        print(f"Train dataset size: {len(train_set)}, valid dataset size: {len(valid_set)}, test dataset size: {len(test_set)}")
    elif args.cm_three_dataset == 'Henan':
        
        test_set = HeNan_ECGBaseDis(data_excel='/home/dingzhengyao/Work/ECG_CMR_TAR/ECG_CMR_Rework/ECG_CMR_CMAI/data/HenanData/all_set.xlsx',isTrain=False,args=args)
        ...
        # zheyi_CMAI_data_path = '/mnt/sda1/dingzhengyao/Work/ECG_CMR_CMAI/Zheyi_Data/zheyi_all_CMAI.pkl'
        # zheyi_data =  pickle.load(open(zheyi_CMAI_data_path, 'rb'))
        # print(len(zheyi_data))
        # test_set = ECGzheyi_three_Base(data=zheyi_data,isTrain=False,args=args)
        ...
        RCM_eid = json.load(open('/home/dingzhengyao/Work/ECG_CMR_TAR/ECG_CMR_Rework/ECG_CMR_CMAI/data/MIMIC/Three/mimic_data_path_RCM.json')) 
        DCM_eid = json.load(open('/home/dingzhengyao/Work/ECG_CMR_TAR/ECG_CMR_Rework/ECG_CMR_CMAI/data/MIMIC/Three/mimic_data_path_DCM.json')) 
        HCM_eid = json.load(open('/home/dingzhengyao/Work/ECG_CMR_TAR/ECG_CMR_Rework/ECG_CMR_CMAI/data/MIMIC/Three/mimic_data_path_HCM.json'))
    ...
    if args.input_modality == 'ECG':
        if args.ecg_model == 'stmem':
            ...
            model = xresnet1d101(num_classes=3, input_channels=12, kernel_size=5,
            ...
            model = ECGModel(output_class_num=3, encoder='resnet50',
