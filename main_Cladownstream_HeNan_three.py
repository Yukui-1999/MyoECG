...
from data.dataset import HeNan_ECGBaseDis,ECGzheyi_three_Base,ECGBaseMIMIC_CMthree,ShaoyifuCardiomyopathyDataset,Harvard_ECGBaseDis
...
def get_args_parser():
    ...
    parser.add_argument('--group_analysis_csv',action='store_true', help='save group analysis csv')
    return parser
...
def main(args):
    ...
    if args.cm_three_dataset == 'Henan':
        ...
        print(f"test dataset size: {len(test_set)}")
    elif args.cm_three_dataset == 'Harvard':
        df_all = pd.read_csv('/home/dingzhengyao/Work/ECG_CMR_TAR/ECG_CMR_Rework/ECG_CMR_CMAI/data/HafoData_adjust2/norm_process/HarvardECG_step6.csv')
       
        test_set = Harvard_ECGBaseDis(data_df=df_all,isTrain=False,args=args)
